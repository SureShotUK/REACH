...
**Last Updated**: 2026-07-15 PM (IMG banner deploy verified in production — all figures extract; padding proven load-bearing via rawPicks; Amelai reboot pending, recovery verified)
...
- **n8n Customer Profiler** — major upgrade 2026-07-14/15: PDF branch runs `qwen3-vl:32b` (correct figures where qwen2.5vl mis-read columns) with page-sized `num_ctx`; filings over 42 pages route through a two-stage survey (banner-labeled 75-DPI pass picks statement pages, then only those convert at full DPI) — **73-page filing now extracts all figures correctly** after three survey fixes (thinking budget `num_predict:16384` with coupled headroom; deterministic +1/+2 page padding in `Pick Pages` because the model conflates printed footer numbers with banner numbers inconsistently; survey cap now 78 pages). IMG banner rename deployed and **verified in production** (rebuild + publish + confirmation run all done): all figures extract, but `rawPicks` shows the model still reports printed footer numbers under IMG banners — **the padding is the load-bearing fix and stays**. Survey thinking volume varies 8× run-to-run (last run grazed `num_predict`: 16,301/16,384); user runs long-filing batches overnight. Remaining: the other three >42-page consultancy companies; brevity levers if truncation recurs. Parser accepts flattened multi-add pastes, zero-pads company numbers, reports skipped lines in chat, and never invents data (ranking:5 fallback removed — errors instead). Live workflow now managed via the n8n API from `n8n/Portland Fuel - Customer Profiler.json` (source of truth, sync-verified); stale June duplicate deleted. Loop node output wiring bug fixed 2026-07-02. New `update` command added 2026-07-02 to `n8n/Main/NewCustomerProfiler.json`; documented in `n8n/Company_Profiler.md`; not yet imported/activated in n8n
...
- **Two-stage survey debugged to full accuracy (2026-07-15)** — three root causes fixed from live n8n execution autopsies: survey thinking outgrowing `num_predict` (raised to 16,384 with provably-coupled context headroom); the model applying next-page rules inconsistently (moved to deterministic +1/+2 padding in `Pick Pages` code); footer/banner page-number conflation (structural fix: banner token renamed to `IMG n OF m`). 73-page GXO filing extracts all six figures correctly; extraction stage confirmed returning honest nulls (never fabricating) when given wrong pages; `padDepth`/`rawPicks` breadcrumbs added for evidence-based padding retirement
